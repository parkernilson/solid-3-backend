...
create table "secrets"."env" (
    "key" text not null,
    "value" text not null
);


CREATE UNIQUE INDEX env_pkey ON secrets.env USING btree (key);

alter table "secrets"."env" add constraint "env_pkey" PRIMARY KEY using index "env_pkey";

grant delete on table "secrets"."env" to "service_role";

grant insert on table "secrets"."env" to "service_role";

grant select on table "secrets"."env" to "service_role";

grant update on table "secrets"."env" to "service_role";
